...
<script>
   $(document).ready(function(){
    ...
    tabel_d_purchasereturn_dt = $('#tabel_d_purchasereturn_dt').DataTable({
      ...
      tabel_d_purchasereturn_dt.on('draw.dt', function(){
        count_prdt_pricetotal();
      });
     ...
     if(purchasereturn_dt.length > 0) {
       for(var x = 0;x < purchasereturn_dt.length;x++) {
         ...
         var prdt_qtyreturn = "<input readonly type='hidden' name='prdt_qtyreturn[]' value='" + data.prdt_qtyreturn + "'>" + parseInt(data.prdt_qtyreturn);
         var s_detname = data.s_detname;
         var prdt_price = parseInt(data.prdt_price) ;
         var prdt_pricetotal = prdt_price * prdt_qtyreturn;
         var stock = data.stock;

         prdt_qty = "<input readonly type='hidden' name='prdt_qty[]' value='" + data.i_id + "'>" + data.prdt_qty;
         ...
         tabel_d_purchasereturn_dt.row.add(
           [prdt_item, prdt_qty, prdt_qtyreturn, s_detname, prdt_price, prdt_pricetotal, stock]
         );
