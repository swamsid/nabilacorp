<script>
  function remove_item(obj) {
    ...
    tabel_d_shop_purchaseplan_dt.row(tr).remove().draw();

  }
	// Function untuk meng-insert shop_purchase plan
  function count_grandtotal() {
      var qty = $('[name="sppdt_qty[]"]');
      var price = $('[name="sppdt_price[]"]');
  ...
  function insert_d_shop_purchase_plan() {
    var data = $('#form_shop_purchase_plan').serialize();
    $.ajax({
      url: "{{ route('insert_d_shop_purchase_plan') }}",
      type: 'POST',
      data: data,
      dataType: 'json',
  ...
  // Function untuk memperbarui shop_purchase plan
  function perbarui_d_shop_purchase_plan() {
    var data = $('#form_shop_purchase_plan').serialize();
    $.ajax({
      url: "{{ route('update_d_shop_purchase_plan') }}",
      type: 'POST',
      data: data,
      dataType: 'json',
      success: function (response) {
        if(response.status == 'sukses') {
          ...
          iziToast.success({
            ...
            onClosing : function() {
              location.href = "{{ route('index_shop_rencanapembelian') }}";
            }
          });
