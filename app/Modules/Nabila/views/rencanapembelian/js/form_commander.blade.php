<script>
	...
	$(document).ready(function(){
		...
		tabel_d_shop_purchaseplan_dt = $('#tabel_d_shop_purchaseplan_dt').DataTable({
			'columnDefs': [
		   		{
		   			"targets": [1, 3, 5],
	   		...
	   		"createdRow": function( row, data, dataIndex ) {
	          		var sppdt_qty = $(row).find('[name="sppdt_qty[]"]');
	          		sppdt_qty.on('change keyup', function(){
	          			var qty = $(this).val();
	          			qty = qty != '' ? qty : 0;
	          			var tr = $(this).parents('tr');
	          			var price = tr.find('[name="sppdt_price[]"]').val();
		...
		$('#qty').keypress(function(e){
			if(e.keyCode == 13) {
				...
				if( $(this).val() == '' || $(this).val() == 0) {
					...
					sppdt_qty = "<input class='text-right' type='number' name='sppdt_qty[]' value='" + sppdt_qty + "'>";
		...
		$('#tabel_d_shop_purchaseplan_dt').on( 'draw.dt', function () {
