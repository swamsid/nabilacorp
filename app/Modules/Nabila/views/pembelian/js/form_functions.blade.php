<script>
  ...
  function count_grandtotal() {
      var qty = $('[name="spodt_qtyconfirm[]"]');
      var price = $('[name="spodt_price[]"]');
  ...
  function insert_d_shop_purchase_order() {
    var data = $('#form_shop_purchase_order').serialize();
    $.ajax({
      url: "{{ route('insert_d_shop_purchase_order') }}",
      type: 'POST',
      data: data,
      dataType: 'json',
  ...
  function hapus_detail(obj) {
    ...
    tabel_d_shop_purchaseorder_dt.row(tr).remove().draw();
  }

  // Function untuk memperbarui shop_purchase order
  function perbarui_d_shop_purchase_order() {
    var data = $('#form_shop_purchase_order').serialize();
    $.ajax({
      url: "{{ route('update_d_shop_purchase_order') }}",
      type: 'POST',
      data: data,
      dataType: 'json',
      success: function (response) {
        if(response.status == 'sukses') {
          ...
          iziToast.success({
            ...
            onClosing : function() {
              location.href = "{{ route('index_shop_pembelian') }}";
            }
          });
  ...
  function find_d_shop_purchaseplan_dt(id) {
    tabel_d_shop_purchaseorder_dt.clear().draw();
    var url = '{{ route("find_d_shop_purchaseplan_dt", ["id" => ""]) }}';
    url = url + "/" + id;
    $.ajax({
      url: url,
      type: 'GET',
      success: function (response) {
        var unit;
        if(response.d_shop_purchaseplan_dt.length > 0) {
          for(x = 0;x < response.d_shop_purchaseplan_dt.length;x++) {
            unit = response.d_shop_purchaseplan_dt[x];
            spodt_item = "<input type='hidden' name='spodt_item[]' value='" + unit.i_id + "'>" + unit.i_code + " - " + unit.i_name; 
            spodt_qty = "<input type='hidden' name='spodt_qty[]' value='" + unit.sppdt_qty + "'>" + unit.sppdt_qty; 
            spodt_qtyconfirm = "<input type='number' name='spodt_qtyconfirm[]' value='" + unit.sppdt_qty + "' class='form-control'>";
            spodt_satuan = "<input type='hidden' name='spodt_satuan[]' value='" + unit.sppdt_satuan + "'>" + unit.s_name; 
            price = unit.sppdt_price;
            ...
            subtotal = unit.sppdt_qty * unit.sppdt_price;
            ...
            tabel_d_shop_purchaseorder_dt.row.add([
              spodt_item, spodt_qty, spodt_qtyconfirm, spodt_satuan, spodt_price, spodt_prevcost, subtotal, s_qty, remove_btn
            ]);
          }
  ...
  function payment() {
    $html = '';
    $html += '<td>' +
      '<input class="minu mx f2 nominal alignAngka nominal' + dataIndex + '" style="width:90%" type="" name="spo_nominal[]"' +
  ...
  function simpanPos(status = '') {
    ...
    $.ajax({
      url: "{{ url('') }}" + '/pembelian/pos-toko/create',
      type: 'GET',
      ...
      success: function (response) {
        ...
        if (response.status == 'sukses') {
          ...
          if (response.s_status == 'final') {
            ...
            $.ajax({
              url: "{{ url('') }}" + '/pembelian/pos-toko/printNota/' + response.s_id,
              type: 'get',
              data: formPos + '&status=' + status,
              success: function (response) {
  ...
  function perbaruiData() {
    ...
    var formPos = $('#dataPos').serialize();
    $.ajax({
      url: "{{ url('') }}" + '/pembelian/pos-toko/update',
      type: 'GET',
      ...
      success: function (response) {
        ...
        if (response.status == 'sukses') {
          ...
          if (response.s_status == 'final') {
            var childwindow = window.open("{{ url('') }}" + '/pembelian/pos-toko/printNota/' + response.s_id, '_blank');
          }
  ...
  function detail(s_id) {
    ...
    dataIndex = 1;
    $.ajax({
      url: "{{ url('') }}" + '/pembelian/pos-toko/' + s_id + '/edit',
      type: 'GET',
      data: {
        "_token": "{{ csrf_token() }}",
        "s_status": statusPos,
