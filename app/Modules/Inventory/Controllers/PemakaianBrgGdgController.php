...
class PemakaianBrgGdgController extends Controller
{
    ...
    public function simpanDataPakai(Request $request, $comp)
    {
        // dd($request->all(), $comp);
        DB::beginTransaction();
        try 
                ...
                if (count($data_sm) > 0) 
                {
                    ...
                    for ($j=0; $j < count($data_sm); $j++) 
                    {
                        ...
                        if ($qty_req <= $qty_sisa) 
                        {
                            ...
                            $dataIsi->d_pbdt_comp = $request->fieldIpScomp[$i];
    ...
    public function getDataDetail($id)
    {
        ...
        $dataIsi = d_pakai_barangdt::join('d_pakai_barang', 'd_pakai_barangdt.d_pbdt_pbid', '=', 'd_pakai_barang.d_pb_id')
            ...
            ->join('m_satuan', 'd_pakai_barangdt.d_pbdt_sat', '=', 'm_satuan.s_id')
            ->select(
