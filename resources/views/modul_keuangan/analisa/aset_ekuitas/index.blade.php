...
<html>
	<head>
		...
		<title>Analisa Aset Terhadap Ekuitas</title>
				...
				<div id="contentnya">
					...
					{{-- Judul Kop --}}
						...
						<table width="100%" border="0" style="border-bottom: 1px solid #333;" v-if="pageNow == 1" v-cloak>
				          <thead>
				            <tr>
				              <th style="text-align: left; font-size: 14pt; font-weight: 600; padding-top: 10px;" colspan="2">Analisa Aset Terhadap Ekuitas</th>
				            </tr>
								...
								<template v-for="(dta, index) in dataPrint">
									<tr>
										<td style="font-weight: 600; text-align: center;">
											@{{ humanizeDate(dta.tanggal).substring(3) }}
										</td>

										<td style="font-weight: 600; text-align: right; font-weight: bold;">
											@{{ humanizePrice(((parseFloat(dta.ekuitas.saldo_awal) + parseFloat(dta.ekuitas.penambahan)) - parseFloat(dta.ekuitas.pengurangan))) }}
										</td>

										<td style="font-weight: 600; text-align: right; font-weight: bold;">
											@{{ humanizePrice((parseFloat(dta.aset.saldo_awal) + parseFloat(dta.aset.penambahan)) - parseFloat(dta.aset.pengurangan)) }}
										</td>

										<td style="font-weight: 600; text-align: center; font-weight: bold;" v-if="(parseFloat(dta.ekuitas.saldo_awal) + parseFloat(dta.ekuitas.penambahan)) - parseFloat(dta.ekuitas.pengurangan) != 0">
											@{{ (((parseFloat(dta.ekuitas.saldo_awal) + parseFloat(dta.ekuitas.penambahan)) - parseFloat(dta.ekuitas.pengurangan)) / ((parseFloat(dta.aset.saldo_awal) + parseFloat(dta.aset.penambahan)) - parseFloat(dta.aset.pengurangan)) * 100).toFixed(2) }} %
										</td>
    	...
    	<script type="text/javascript">
			...
			var app = 	new Vue({
				            ...
				            mounted: function(){
				            	...
				            	axios.get('{{route('analisa.keuangan.aset_ekuitas.data_resource')}}?'+that.url.searchParams)
			                            .then((response) => {
				            ...
				            methods: {
				            	...
				            	downloadPdf: function(evt){
				                    ...
				                    $('#pdfIframe').attr('src', '{{route('analisa.keuangan.aset_ekuitas.print.pdf')}}?'+that.url.searchParams)
				            	...
				            	print: function(evt){
				            		...
				            		// $('#pdfIframe').attr('src', '{{route('analisa.keuangan.aset_ekuitas.print')}}?'+that.url.searchParams)
				            	},
