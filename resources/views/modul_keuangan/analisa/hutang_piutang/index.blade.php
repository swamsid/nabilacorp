...
<html>
	<head>
		...
		<title>Analisa Hutang Piutang</title>
						...
						<table class="table" id="table-data" v-cloak>
							<thead>
								<tr>
									...
									<th width="40%" colspan="2" style="background-color: #0099CC; color: white;">Piutang</th>
									<th width="40%" colspan="2" style="background-color: #0099CC; color: white;">Hutang</th>
								</tr>

								<tr>
									<th style="text-align: center; background-color: #0099CC; color: #ffffff;">Piutang Baru</th>
									<th style="text-align: center; background-color: #0099CC; color: #ffffff;">Sudah Dibayar</th>


									<th style="text-align: center; background-color: #0099CC; color: #ffffff;">Hutang Baru</th>
									<th style="text-align: center; background-color: #0099CC; color: #ffffff;">Sudah Dibayar</th>
								</tr>
							</thead>
								...
								<template v-for="(dta, index) in dataPrint">
									<tr>
										...
										<td style="font-weight: 600; text-align: right;">
											@{{ humanizePrice(dta.piutang.total_tagihan) }}
										</td>

										<td style="font-weight: 600; text-align: right;">
											@{{ humanizePrice(dta.piutang.sudah_dibayar) }}
										</td>

										<td style="font-weight: 600; text-align: right;">
											@{{ humanizePrice(dta.hutang.total_tagihan) }}
										</td>

										<td style="font-weight: 600; text-align: right;">
											@{{ humanizePrice(dta.hutang.sudah_dibayar) }}
										</td>
									</tr>
								</template>	
							</tbody>
    	...
    	<script type="text/javascript">
			...
			var app = 	new Vue({
				            ...
				            mounted: function(){
				            	...
				            	axios.get('{{route('analisa.keuangan.hutang_piutang.data_resource')}}?'+that.url.searchParams)
			                            .then((response) => {
				            ...
				            methods: {
				            	...
				            	downloadPdf: function(evt){
				                    ...
				                    $('#pdfIframe').attr('src', '{{route('analisa.keuangan.hutang_piutang.print.pdf')}}?'+that.url.searchParams)
				            	...
				            	print: function(evt){
				            		...
				            		// $('#pdfIframe').attr('src', '{{route('analisa.keuangan.hutang_piutang.print')}}?'+that.url.searchParams)
				            	},
